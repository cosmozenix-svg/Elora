...
import { 
  Send, PlusCircle, Smartphone, Landmark, Clock, UserCircle, 
...
export default function Dashboard() {
  ...
  const options = [
    ...
    { title: 'Recharge', icon: Smartphone, path: '/recharge', color: 'bg-amber-500 text-white' },
    { title: 'Withdraw', icon: Landmark, path: '/withdraw', color: 'bg-rose-600 text-white' },
    ...
    { title: 'History', icon: HistoryIcon, path: '/history', color: 'bg-slate-700 text-white' },
    ...
    <div className="space-y-4">
              ...
              <div className="text-left">
                {showBalance ? (
                  <span className="text-2xl font-black tracking-tight font-mono">
                    {currentUser.balance.toFixed(2)} <span className="text-sm font-semibold text-blue-200 dark:text-blue-300">BDT</span>
                  </span>
                ) : (
        ...
        <div className="grid grid-cols-4 gap-2.5">
          {options.map((opt) => (
            <Link
              key={opt.title}
              to={opt.path}
              className="flex flex-col items-center justify-center p-2.5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-xs hover:border-blue-300 dark:hover:border-blue-600/60 active:scale-95 transition-all group"
            >
              <div className={cn(
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/80 dark:border-slate-800 p-4 shadow-xs transition-colors">
        <div className="flex justify-between items-center mb-3">
          ...
          <Link to="/history" className="text-[11px] font-bold text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 flex items-center gap-1">
            ...
            {userTransactions.map((tx) => (
              <div key={tx.id} className="py-2.5 flex items-center justify-between first:pt-0 last:pb-0">
                <div className="flex items-center gap-2.5">
                  <div className={cn(
                    ...
                    {tx.type === 'send' ? <ArrowUpRight className="w-4 h-4" /> :
                     ...
                     tx.type === 'recharge' ? <Smartphone className="w-4 h-4" /> :
                ...
                <span className={cn(
                  ...
                  {['receive', 'add', 'earn'].includes(tx.type) ? '+' : '-'}{tx.amount.toFixed(2)} BDT
                </span>
              </div>
            ))}
          </div>
